...
class LISTALSTM(nn.Module):
    def __init__(self, m, n, W_e, L, theta, max_iter):
        ...
        super(LISTALSTM, self).__init__()
        self._W = nn.Linear(in_features=m, out_features=n, bias=False)
        self._S = nn.Linear(in_features=n, out_features=n,
                            bias=False)

        self.lstmcell = nn.LSTMCell(input_size=n, hidden_size=n, bias=False)
        self.coef = nn.Linear(in_features=n, out_features=n, bias=False)
    ...
    def forward(self, y):
        ...
        for iter in range(self.max_iter):
            h, c = self.lstmcell(h)
            h = self.shrinkage(self._W(y) + h)

        return h


def train_listalstm(Y, dictionary, a, L, epochs=100):
    ...
    theta = a/L
    net = LISTALSTM(m, n, W_d, L, theta, max_iter=30)
    net.weights_init()
    net = net.cuda()
