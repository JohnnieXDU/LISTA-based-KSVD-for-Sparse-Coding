...
import torch
from image_inpainting.inpainting_ista import inpainting_ista_once


def KSVD_ISTA_Inpainting(D, Y, maskvec, n_non_zeros, img_ori, blksize, overlap, maxiter=10, etol=1e-10, approx=False):
    """
    ...
    for iteration in range(1, maxiter + 1):
        ...
        print('  -> update coefficient: ISTA')

        X = inpainting_ista_once(Y, D, maskvec, n_nonzero_coefs=50)
...
def inpainting_ksvd_ista(missing_pixels, dicsize, n_nonzero_coefs, blksize, overlap, maxiter, grayimg=False):
    ...
    if load_results:
        ...
        if grayimg:
            ...
            Y = patchvec
            newD, coef = inpainting_ksvd_ista_once(Y, D, maskvec, img_ori, blksize, overlap, n_nonzero_coefs=50,
                                                    maxiter=maxiter)
...
def inpainting_ksvd_ista_once(Y, D, maskvec, img_ori, blksize, overlap, n_nonzero_coefs, maxiter):
    # KSVD to update dict and coef
    new_D, new_coef, error = KSVD_ISTA_Inpainting(D, Y, maskvec, n_nonzero_coefs,
