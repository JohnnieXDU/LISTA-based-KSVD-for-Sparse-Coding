...
def KSVD_Inpainting(D, Y, maskvec, n_non_zeros, sigma=0.001, rc_min=0.01, maxiter=10, etol=1e-10, approx=False):
    """
    ...
    for iteration in range(1, maxiter + 1):
        ...
        # X = OrthogonalMatchingPursuit_Inpainting(D, Y, n_non_zeros, sigma, rc_min)
        X = inpainting_omp_once(Y, D, maskvec, n_non_zeros, sigma=sigma, rc_min=rc_min)
...
def inpainting_ksvd_once(Y, D, maskvec, n_nonzero_coefs, maxiter):
    # KSVD to update dict and coef
    new_D, new_coef, error = KSVD_Inpainting(D, Y, maskvec, n_nonzero_coefs,
                                                 sigma=0.001, rc_min=0.01,
